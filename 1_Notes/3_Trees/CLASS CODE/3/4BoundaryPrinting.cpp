#include<bits/stdc++.h>
using namespace std;
class node{
public:
    int val;
    node* left;
    node* right;
    node(int val) {
        this->val = val;
        right = NULL;
        left = NULL;
    }
};
void BFS_printing(node* root) { //prints level wise
    queue<node*> q;
    q.push(root);
    while(q.size() > 0) {
        node*temp = q.front();
        q.pop();
        cout << temp->val << " ";
        if(temp->left != NULL) q.push(temp->left);
        if(temp->right != NULL) q.push(temp->right);
    }
    cout << endl;
}
node* ArrayToTree(int arr[], int n) {
    queue<node*> q;
    node* root = new node(arr[0]);
    q.push(root);

    int i = 1;
    int j = 2;
    while(q.size() > 0 && i < n) {
        node* temp = q.front();
        q.pop();
        node* l;
        node* r;

        if(arr[i] != INT_MIN) l = new node(arr[i]);
        else l = NULL;
        if(j != n && arr[j] != INT_MIN) r = new node(arr[j]);
        else r = NULL;

        temp->left = l;
        temp->right = r;

        if(l != NULL) q.push(l);
        if(r != NULL) q.push(r);

        i += 2;
        j += 2;
    }
    return root;
}
void leftboundaryPrinting(node* root) {
    if(root == NULL) return ;
    if(root->left == NULL and root->right == NULL) return;
    cout << root->val << " ";
    leftboundaryPrinting(root->left);
    if(root->left == NULL) leftboundaryPrinting(root->right);
}
void leafboundaryPrinting(node* root) {
    if(root == NULL ) return ;
    if(root->left == NULL and root->right == NULL){
        cout << root->val << " ";
        return ;
    }
    leafboundaryPrinting(root->left);
    leafboundaryPrinting(root->right);
}
void rightboundaryPrinting(node* root) {
    if(root == NULL ) return ;
    if(root->left == NULL && root->right == NULL) return;
    rightboundaryPrinting(root->right);
    if(root->right == NULL) rightboundaryPrinting(root->left);
    cout << root->val << " ";
}
int main() {
    int arr[] = {1, 2, 3, 4, 5, INT_MIN, 6, INT_MIN, INT_MIN, 7, 8, 9};
    node* root = ArrayToTree(arr, 12);
    cout << endl;
    cout << "BFS_printing -> ";
    BFS_printing(root);

    cout << endl;
    cout << "Following are the boundary printing : " << endl;
    leftboundaryPrinting(root);
    cout << endl;
    leafboundaryPrinting(root);
    cout << endl;
    rightboundaryPrinting(root->right);
    cout << endl;
    cout << endl;
}